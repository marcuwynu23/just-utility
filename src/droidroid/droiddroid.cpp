...
void show() {
  cout << WHITE_BOLD +
              " DroidDroidHelper (Android Development on Android Device Helper)"
       << endl;
  cout << GREEN_BRIGHT + "help" + RESET + "     show this message" << endl;
  cout << GREEN_BRIGHT + "cls" + RESET + "      clear screen" << endl;
  cout << GREEN_BRIGHT + "net" + RESET +
              "       connect via net(default port: 7575)"
       << endl;
  ...
  cout << GREEN_BRIGHT + "pt " + RESET +
              "       enable/disable pointer location of devices"
       << endl;
  cout << GREEN_BRIGHT + "th " + RESET + "       enable/disable show touches "
       << endl;
  cout << GREEN_BRIGHT + "cmd" + RESET + "       open commandline Terminal"
       << endl;
  cout << GREEN_BRIGHT + "e  " + RESET + "       open text Editor" << endl;
  cout << GREEN_BRIGHT + "lsp" + RESET + "       list projects" << endl;
  cout << GREEN_BRIGHT + "psa" + RESET +
              "       push projects to remote android device directory"
       << endl;
  cout << GREEN_BRIGHT + "pla" + RESET +
              "       pull projects from remote android device directory"
       << endl;
  cout << GREEN_BRIGHT + "ps " + RESET +
              "       push a project to remote android device directory"
       << endl;
  cout << GREEN_BRIGHT + "pl " + RESET +
              "       pull a project from remote android device directory"
       << endl;
  cout << GREEN_BRIGHT + "dpl" + RESET +
              "       delete project in the local directory"
       << endl;
  cout << GREEN_BRIGHT + "dpr" + RESET +
              "       delete project in the remote directory"
       << endl;
  cout << GREEN_BRIGHT + "ut " + RESET +
              "       uninstall android application from android device"
       << endl;
  cout << GREEN_BRIGHT + "it " + RESET +
              "       install android package to the android device"
       << endl;
  cout << GREEN_BRIGHT + "ra " + RESET +
              "       run android application from android device"
       << endl;
  cout << GREEN_BRIGHT + "ca " + RESET +
              "       close android application from android device"
       << endl;
  cout << GREEN_BRIGHT + "sc " + RESET + "       save and compile " << endl;
  cout << GREEN_BRIGHT + "ir " + RESET + "       install and run" << endl;
}

void openShell() { system("adb shell"); }
...
void connectViaUSB() { runExe("adb usb "); }
...
void pullprojects(string LOCAL_PATH, string REMOTE_PATH, string WORK_DIR) {
  cout << " pull projects from " + REMOTE_PATH + "/" + WORK_DIR + "... "
       << endl;
...
void pullproject(string LOCAL_PATH, string REMOTE_PATH, string WORK_DIR) {
  ...
  cout << "pull " + PROJECT_NAME + " from " + REMOTE_PATH + "/" + WORK_DIR +
              "/" + PROJECT_NAME + "..."
       << endl;
  runExe("adb pull " + REMOTE_PATH + "/" + WORK_DIR + "/" + PROJECT_NAME + " " +
         LOCAL_PATH + "\\" + WORK_DIR);
...
void pushproject(string LOCAL_PATH, string REMOTE_PATH, string WORK_DIR) {
  ...
  cout << "Enter project name: ";
  getline(cin, PROJECT_NAME);
  runExe("adb shell rm -rf " + REMOTE_PATH + "/" + WORK_DIR + "/" +
         PROJECT_NAME);
  cout << "echo pushing " + PROJECT_NAME + " to remote directory... " << endl;
  runExe("adb push " + LOCAL_PATH + "\\" + WORK_DIR + "\\" + PROJECT_NAME +
